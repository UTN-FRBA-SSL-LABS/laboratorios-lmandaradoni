...
> R:Resetea el carrito que se le pasa por parametro, poniendo su cantidad de productos en cero
...
> R:La funcion suma todos los precios de los items, pero no los multiplica por sus cantidades.
...
> R:Hay un <= donde deberia haber solo un <
...
> R:Si, no se ejecuto la funcion carrito_descuento. Linea 29 y 30.
...
> R:La diferencia es que uno hace tests individuales de cada funcion, en cambio los de integracion testean varias funciones al mismo tiempo.
...
> R:Es peligroso porque C no controla la memoria automaticamente como lo hace Python o Java.
...
> R:Es mejor para ver el funcionamiento del codigo al simular escenarios limite. Permite encontrar los errores mas facilmente.
...
> R:Porque en el primer test se contemplaba un escenario donde se agregaba un unico producto al carrito, por lo cual, no era posible ver el bug en el que no se multiplicaba por la cantidad. En cambio en el segundo test se intentan agregar 2 unidades de un producto, haciendo que se dispare el error por la falta del multiplicador de cantidades.
